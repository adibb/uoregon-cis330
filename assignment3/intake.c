...
/*
    ...
    - board game_board: The board the game is being played
      on. Its size and the current state of the tiles
      dictate which positions are valid or not.
*/
void getPosition(int *row, int *col, board game_board){
    ...
    while (!valid){
        ...
        *row = line[0] - '0' - 1;
        *col = line[2] - '0' - 1;
        printf("POSITION HAS: %d\n", game_board.tiles[*row][*col]);
        printf("BLANK IS: %d\n", BLANK);
        printf("EQUIVALENCE OF FETCHED: %d\n", BLANK == game_board.tiles[*row][*col]);
        printf("EQUIVALENCE OF RAW CHARS: %d\n", '.' == '.');
        valid = (line[1] == ' ') &&
                (line[3] == '\n') &&
